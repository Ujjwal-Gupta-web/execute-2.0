            ...
            <div class="input-group relative flex items-stretch w-full mb-4">
...

              {
                data.filter(post => {
                  ...
                  else if(post.name.toLowerCase().includes(query.toLocaleLowerCase())){
                  return post;
                }
                }).map((post, i)=>(
                  <div className="text-left m-1 w-2/5 pl-3" key={i}>
                    <div className="flex flex-col">
                    <button onclick={(e)=> setCity(e.target.value)} className="text-white text-xl">{post.name}</button>
                    </div>
                    </div>
